...
export default function ViewDataMain({data, activeSectionIndex, activeSection, isLoading}: {data?: ViewDataApiResponse | null, activeSectionIndex: number, activeSection?: Section | null, isLoading?: boolean}) {
    ...
    const sectionName = isLoading ? "" : (activeSection?.section_name || (activeSection as any)?.title || "Extracted Section");
            ...
            <div className="max-w-[98%] mx-auto animate-in slide-in-from-right-4 duration-300">
                    ...
                    <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-white">
                        {isLoading ? (
                            ...
                            <>
                                ...
                                <span className="text-xs text-gray-400 font-bold uppercase tracking-tight">{fields.length} Points Extracted</span>
                            </>
                        )}
                    </div>
                            ...
                            <TableHeader className="bg-gray-50/80 text-gray-500 uppercase text-[11px] font-bold tracking-wider">
                                <TableRow>
                                    {isLoading?(
                                        ...
                                        <>
                                            <TableHead className="px-8 py-4 w-1/4 h-auto font-bold text-gray-500">Field Name</TableHead>
                                            <TableHead className="px-8 py-4 w-1/2 h-auto font-bold text-gray-500">Extracted Value</TableHead>
                                            <TableHead className="px-8 py-4 h-auto font-bold text-gray-500">Source Reference</TableHead>
                                        </>
                                    )}
                            ...
                            <TableBody className="divide-y divide-gray-100">
                                {isLoading ? (
                                    ...
                                    fields.map((field: Field, idx: number) => {
                                        ...
                                        const fieldSource = field.citation_text || (field as any).source || (field.citation_pages?.length ? `Pages: ${field.citation_pages.join(', ')}` : "No direct citation");

                                        return (
                                            <TableRow key={idx} className="hover:bg-blue-50/30 transition-colors group border-b border-gray-100">
                                                <TableCell className="px-8 py-5 font-bold text-gray-700 whitespace-nowrap">{fieldName}</TableCell>
                                                <TableCell className="px-8 py-5 text-gray-900 font-medium leading-relaxed">
                                                    {fieldValue || <span className="text-gray-300 italic font-normal">N/A</span>}
                                                </TableCell>
                                                <TableCell className="px-8 py-5 text-gray-500 italic whitespace-nowrap text-xs">
                                                    <div className="flex flex-col gap-1">
                                                        ...
                                                        <p className="text-[10px] text-gray-400 font-medium leading-tight ">
                                                            Extracted from primary disclosure text in the identified document segment.
                                                        </p>
                                                    </div>
                                                </TableCell>
                                            </TableRow>
                ...
                {!isLoading && data?.sections && (
                    <div className="mt-8 flex items-center justify-center gap-4 text-gray-400 text-[10px] font-bold uppercase tracking-widest py-4 border-t border-gray-200 border-dashed">
                        <p>Section {activeSectionIndex + 1} of {data.sections.length}</p>
                    </div>
                )}
            </div>
        </main>
